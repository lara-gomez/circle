...
Deno.test("ReviewingConcept: addReview - Requirements and Effects", async () => {
  ...
  try {
    ...
    const addInvalidRatingLowResultError = addInvalidRatingLowResult as { error: string }
    assertEquals(addInvalidRatingLowResultError.error, "Rating must be between 0 and 10.");
    console.log(`Requirement: Failed as expected: ${addInvalidRatingLowResultError.error}`);
    ...
    const addInvalidRatingHighResultError = addInvalidRatingHighResult as { error: string }
    assertEquals(addInvalidRatingHighResultError.error, "Rating must be between 0 and 10.");
    console.log(`Requirement: Failed as expected: ${addInvalidRatingHighResultError.error}`);
    ...
    const addDuplicateResultError = addDuplicateResult as { error: string };
    assertEquals(addDuplicateResultError.error, `User ${userAlice} has already reviewed item ${itemProductA}.`);
    console.log(`Requirement: Failed as expected: ${addDuplicateResultError.error}`);
...
Deno.test("ReviewingConcept: modifyReview - Requirements and Effects", async () => {
  ...
  try {
    ...
    const modifyInvalidRatingLowResultError = modifyInvalidRatingLowResult as { error: string };
    assertEquals(modifyInvalidRatingLowResultError.error, "Rating must be between 0 and 10.");
    console.log(`Requirement: Failed as expected: ${modifyInvalidRatingLowResultError.error}`);
    ...
    const modifyInvalidRatingHighResultError = modifyInvalidRatingHighResult as { error: string };
    assertEquals(modifyInvalidRatingHighResultError.error, "Rating must be between 0 and 10.");
    console.log(`Requirement: Failed as expected: ${modifyInvalidRatingHighResultError.error}`);
    ...
    const modifyNonExistentResultError = modifyNonExistentResult as { error: string };
    assertEquals(modifyNonExistentResultError.error, `No review by user ${userBob} for item ${nonExistentItem} found to modify.`);
    console.log(`Requirement: Failed as expected: ${modifyNonExistentResultError.error}`);
