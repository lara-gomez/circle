...
content_id: 29a39f049d4ff167410821d8ba62c712f67ac2048f0304746d04143cc2997214
---
...
export const RequestGetEventsByOrganizer: Sync = ({
  ...
  where: async (frames) => {
    ...
    frames = await frames.query(Event._getEventsByOrganizer, { organizer }, { event: eventDoc });
    if (frames.length === 0) {
      return emptyResultsFrame(original, results);
    }

    await markCompletedIfNeeded(frames, eventDoc);
    return frames.collectAs([eventDoc], results);
  },
...
export const RequestGetEventById: Sync = ({
  ...
  where: async (frames) => {
    ...
    frames = await frames.query(Event._getEventById, { event: eventId }, { event: eventDoc });
    if (frames.length === 0) {
      return new Frames({ ...original, [result]: null });
    }

    await markCompletedIfNeeded(frames, eventDoc);
    const doc = frames[0][eventDoc];
    return new Frames({ ...frames[0], [result]: doc });
  },
  ...
  where: async (frames) => {
    ...
    for (const frame of frames) {
      ...
      if (Array.isArray(result)) {
        ...
        outputs.push({ ...frame, [results]: result, [recommendationError]: undefined });
      } else {
        outputs.push({ ...frame, [results]: [], [recommendationError]: result.error });
      }
    }
    if (outputs.length === 0) {
      return emptyResultsFrame(original, results);
...
export const RequestGetEventsByStatus: Sync = ({
  ...
  where: async (frames) => {
    ...
    frames = await frames.query(Event._getEventsByStatus, { status }, { event: eventDoc });
    if (frames.length === 0) {
      return emptyResultsFrame(original, results);
    }
...
export const RequestGetAllEvents: Sync = ({
  ...
  where: async (frames) => {
    ...
    frames = await frames.query(Event._getAllEvents, {}, { event: eventDoc });
    if (frames.length === 0) {
      return emptyResultsFrame(original, results);
    }
