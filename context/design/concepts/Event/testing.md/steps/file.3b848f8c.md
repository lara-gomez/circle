...
content_id: 3b848f8c7a9032db0303bd26b9d5d8f552a1444acec146959fc3b6f241c23c3b
---
...
Deno.test("Principle: User schedules, event completes, organizer cancels/uncancels/deletes", async () => {
  ...
  try {
    ...
    assertEquals("error" in getEvent1Result1, false, `Expected event to be found, got error: ${JSON.stringify(getEvent1Result1)}`);
    ...
    assertEquals("error" in getPastEventResult, false, `Expected completed event to be found, got error: ${JSON.stringify(getPastEventResult)}`);
    ...
    assertEquals("error" in getEvent1Result2, false, `Expected event to be found after cancellation, got error: ${JSON.stringify(getEvent1Result2)}`);
    ...
    assertEquals("error" in getEvent1Result3, false, `Expected event to be found after un-cancellation, got error: ${JSON.stringify(getEvent1Result3)}`);
    ...
    // Verify deletion: attempting to retrieve it should now return an error
    const deletedEventCheck = await eventConcept._getEventById({ event: event1Id });
    assertEquals("error" in deletedEventCheck, true, "Retrieving deleted event should result in an error.");
    assertEquals((deletedEventCheck as unknown as { error: string }).error, `Event with ID ${event1Id} not found.`, "Error message should confirm event not found.");
    console.log(`Verification: Event ${event1Id} confirmed as deleted (retrieval attempt returned error).`);
...
Deno.test("Action: createEvent - success and preconditions", async () => {
  ...
  try {
    ...
    assertEquals("error" in fetchedEventResult, false, `Expected created event to be found, got error: ${JSON.stringify(fetchedEventResult)}`);
...
Deno.test("Action: modifyEvent - success and preconditions", async () => {
  ...
  try {
    ...
    assertEquals("error" in modifiedEventResult, false, `Expected modified event to be found, got error: ${JSON.stringify(modifiedEventResult)}`);
...
Deno.test("Action: cancelEvent - success and preconditions", async () => {
  ...
  try {
    ...
    assertEquals("error" in cancelledEventResult, false, `Expected cancelled event to be found, got error: ${JSON.stringify(cancelledEventResult)}`);
    ...
    // Use substring match for error message due to dynamic ID
    assertEquals((alreadyCancelledResult as { error: string }).error.includes("cannot be cancelled as its status is 'cancelled'"), true);
    ...
    assertEquals("error" in completedEventResult, false, `Expected completed event to be found, got error: ${JSON.stringify(completedEventResult)}`);
    ...
    // Use substring match for error message due to dynamic ID
    assertEquals((cancelCompletedResult as { error: string }).error.includes("cannot be cancelled as its status is 'completed'"), true);
...
Deno.test("Action: unCancelEvent - success and preconditions", async () => {
  ...
  try {
    ...
    assertEquals("error" in eventToUncancelResult1, false, `Expected event to be found, got error: ${JSON.stringify(eventToUncancelResult1)}`);
    ...
    assertEquals("error" in eventToUncancelResult2, false, `Expected event to be found, got error: ${JSON.stringify(eventToUncancelResult2)}`);
    ...
    assertEquals("error" in eventToUncancelResult3, false, `Expected event to be found, got error: ${JSON.stringify(eventToUncancelResult3)}`);
    ...
    // Use substring match for error message due to dynamic ID
    assertEquals((uncancelUpcomingResult as { error: string }).error.includes("cannot be un-cancelled as its status is 'upcoming'"), true);
    ...
    assertEquals("error" in pastCancelledEventResult, false, `Expected event to be found, got error: ${JSON.stringify(pastCancelledEventResult)}`);
...
Deno.test("Action: deleteEvent - success and preconditions", async () => {
  ...
  try {
    ...
    assertEquals("error" in deletedEvent, true, "Retrieving deleted event should result in an error."); // Corrected check
    assertEquals((deletedEvent as unknown as { error: string }).error, `Event with ID ${eventId} not found.`, "Error message should confirm event not found."); // Corrected type assertion
...
Deno.test("Action: completeEvent - success and preconditions (system action)", async () => {
  ...
  try {
    ...
    assertEquals("error" in eventToCompleteResult1, false, `Expected event to be found, got error: ${JSON.stringify(eventToCompleteResult1)}`);
    ...
    assertEquals("error" in eventToCompleteResult2, false, `Expected completed event to be found, got error: ${JSON.stringify(eventToCompleteResult2)}`);
    ...
    // Use substring match for error message due to dynamic ID
    assertEquals((alreadyCompletedResult as { error: string }).error.includes("cannot be completed as its status is 'completed'"), true);
    ...
    // Use substring match for error message due to dynamic ID
    assertEquals((completeCancelledResult as { error: string }).error.includes("cannot be completed as its status is 'cancelled'"), true);
...
Deno.test("Queries: _getEventById, _getEventsByOrganizer, _getEventsByStatus, _getAllEvents", async () => {
  ...
  try {
    ...
    assertEquals("error" in fetchedAliceUpcoming, false, `Expected Alice's event to be found, got error: ${JSON.stringify(fetchedAliceUpcoming)}`);
    ...
    assertEquals("error" in fetchedNonExistent, true, "Should return error for non-existent event.");
    assertEquals((fetchedNonExistent as unknown as { error: string }).error, `Event with ID ${nonExistentEventId} not found.`); // Fixed type assertion
    console.log("Success: Handled non-existent event by ID (returned error).");
