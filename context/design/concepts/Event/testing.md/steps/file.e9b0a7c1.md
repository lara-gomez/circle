...
content_id: e9b0a7c1ef735545c342b90326bbdf011d596d6c36ce20601c034ca83e3ec695
---
...
Deno.test("EventConcept: Principle - User organizes, tracks, and manages event lifecycle", async () => {
  ...
  try {
    ...
    const now = new Date();
    // Event 1: for cancellation/uncancellation. Starts ~5 seconds from now, lasts 1 minute.
    const event1FutureDate = new Date(now.getTime() + 5 * 1000); 
    const event1Duration = 1; // 1 minute
    // Event 2: for completion. Starts ~10 seconds from now, lasts 1 minute. We will sleep past its end.
    const event2FutureDate = new Date(now.getTime() + 10 * 1000); 
    const event2Duration = 1; // 1 minute
    ...
    let event2Doc = (await eventConcept._getEventById({ event: event2 }))[0];
    ...
    // We created event2 to be completed, so we wait for its end time to pass.
    console.log(`4. Waiting for Event2 (${event2}) to become eligible for completion (ends at ~${new Date(event2FutureDate.getTime() + event2Duration * 60 * 1000).toLocaleTimeString()})...`);
    // Calculate time to wait, ensuring it's always positive and includes a buffer
    const timeToWait = (event2FutureDate.getTime() + event2Duration * 60 * 1000) - new Date().getTime() + 1000; // Add 1 second buffer
    if (timeToWait > 0) {
...
Deno.test("Action: cancelEvent - success and validation", async () => {
  ...
  try {
    ...
    const now = new Date();
    const pastEventTime = new Date(now.getTime() + 1000); // starts 1 second from now
    ...
    // Wait for the event to pass its end time to be eligible for completion
    const timeToWait = (pastEventTime.getTime() + pastEventDuration * 60 * 1000) - now.getTime() + 1000;
    if (timeToWait > 0) {
      await sleep(timeToWait);
    }

    await eventConcept.completeEvent({ event: pastEventId }); // Complete it first
    const completedEvent = (await eventConcept._getEventById({ event: pastEventId }))[0];
    assertEquals(completedEvent.status, "completed", "Event should be completed.");

    const cancelCompletedResult = await eventConcept.cancelEvent({ organizer: organizerA, event: pastEventId });
...
Deno.test("Action: unCancelEvent - success and validation", async () => {
  ...
  try {
    ...
    // Create an event that starts in the future, then cancel it, then wait for it to end.
    ...
    // Wait for event to pass its end time
    const timeToWait = (pastEndTestStart.getTime() + pastEndTestDuration * 60 * 1000) - new Date().getTime() + 1000;
    if (timeToWait > 0) {
      await sleep(timeToWait);
    }
...
Deno.test("Action: system completeEvent - success and validation", async () => {
  ...
  try {
    const now = new Date();
    // Event 1: starts 1 second from now, lasts 1 minute.
    const futureEventTimeShort = new Date(now.getTime() + 1 * 1000); 
    ...
    console.log(`Waiting for Event1 (${eventId1}) to become eligible for completion (ends at ~${new Date(futureEventTimeShort.getTime() + shortDuration * 60 * 1000).toLocaleTimeString()})...`);
    // Wait for event1 to pass its end time. Add a buffer for reliability.
    const timeToWait1 = (futureEventTimeShort.getTime() + shortDuration * 60 * 1000) - new Date().getTime() + 1000; // 1 second buffer
    if (timeToWait1 > 0) {
      await sleep(timeToWait1);
    }
    ...
    // Wait for it to end after cancellation
    const timeToWait2 = (new Date(now.getTime() + 1 * 1000).getTime() + 1 * 60 * 1000) - new Date().getTime() + 1000;
    if (timeToWait2 > 0) {
      await sleep(timeToWait2);
    }
...
Deno.test("Queries: _getEventById, _getEventsByOrganizer, _getEventsByStatus, _getAllEvents", async () => {
  ...
  try {
    ...
    // Calculate precise time to wait for eventA1 to end + buffer
    const eventA1EndTime = new Date(event1Date.getTime() + 30 * 60 * 1000);
    const timeToWaitAfterCreate = eventA1EndTime.getTime() - new Date().getTime() + 1000; // 1 second buffer
    if (timeToWaitAfterCreate > 0) {
      await sleep(timeToWaitAfterCreate);
    } // If negative, it means event already ended, proceed.
