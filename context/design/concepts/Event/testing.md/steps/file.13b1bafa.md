...
content_id: 13b1bafaac2e15b39fa51c5a3c7b57682416e44bb836346f743724f0a9c354f8
---
...
Deno.test("Principle: User schedules, event completes, organizer cancels/uncancels/deletes", async () => {
  ...
  try {
    ...
    const pastDateForCompletion = new Date(Date.now() - 2 * 60 * 60 * 1000); // 2 hours ago
    pastDateForCompletion.setHours(8, 0, 0, 0); // Normalize time
    ...
    // Verify deletion: attempting to retrieve it should now return an empty array as per query spec
    const deletedEventCheck = await eventConcept._getEventById({ event: event1Id });
    assertEquals(deletedEventCheck.length, 0, "Retrieving deleted event should result in an empty array.");
    console.log(`Verification: Event ${event1Id} confirmed as deleted (retrieval attempt returned empty array).`);
...
Deno.test("Action: modifyEvent - success and preconditions", async () => {
  ...
  try {
    ...
    // The precondition `at least one field must differ from the original event details` is not strictly enforced by implementation.
    // MongoDB updateOne will return modifiedCount 0 if new values are identical to existing,
    // which our current logic translates into a generic error message "could not be updated."
    // If strict enforcement is needed, an explicit comparison before update is required.
    console.log("Test: Behaviour for no actual changes (current implementation returns a generic error if no modifiedCount).");
    ...
    assertEquals("error" in noChangeResult, true, "Expected an error if no fields differ (due to modifiedCount === 0).");
    assertEquals((noChangeResult as { error: string }).error, `Event with ID ${eventId} could not be updated.`, "Error message should reflect no actual changes."); // Original message was slightly different, adjusted for actual code
    console.log("Success: Current implementation correctly indicates no changes were applied.");
...
Deno.test("Action: cancelEvent - success and preconditions", async () => {
  ...
  try {
    ...
    assertEquals((alreadyCancelledResult as { error: string }).error, "Event cannot be cancelled as its status is not 'upcoming'."); // Corrected error message
    ...
    assertEquals((cancelCompletedResult as { error: string }).error, "Event cannot be cancelled as its status is not 'upcoming'."); // Corrected error message
...
Deno.test("Action: unCancelEvent - success and preconditions", async () => {
  ...
  try {
    ...
    // --- Setup for successful un-cancellation ---
    const createResult = await eventConcept.createEvent({
      organizer: organizerAlice,
    ...
    assertEquals(eventToUncancel.status, "upcoming", "Event status should be 'upcoming' after un-cancellation.");
    console.log("Success: Event un-cancelled and verified.");

    // --- Failure: un-cancelling a non-existent event ---
    console.log("Test: Failure - non-existent event.");
    const nonExistentId = "event:fake" as ID;
    ...
    // --- Failure: un-cancelling with unauthorized organizer ---
    console.log("Test: Failure - unauthorized organizer.");
    // Re-cancel the event for this test case
    await eventConcept.cancelEvent({ organizer: organizerAlice, event: eventId }); // Revert to cancelled for unauthorized test
    const unauthorizedUncancelResult = await eventConcept.unCancelEvent({ organizer: organizerBob, event: eventId });
    assertEquals("error" in unauthorizedUncancelResult, true, "Should fail for unauthorized organizer.");
    assertEquals((unauthorizedUncancelResult as { error: string }).error, "Only the event organizer can un-cancel the event.");
    console.log("Success: Rejected unauthorized un-cancellation.");

    // --- Failure: un-cancelling an event that is already upcoming (not cancelled) ---
    console.log("Test: Failure - un-cancelling an already upcoming event.");
    const upcomingEventForUncancel = new Date(Date.now() + 1000 * 60 * 60 * 4); // 4 hours from now
    upcomingEventForUncancel.setHours(12, 0, 0, 0); // Normalize time
    const createUpcomingResult = await eventConcept.createEvent({
      organizer: organizerAlice, name: "Truly Upcoming Event", date: upcomingEventForUncancel, duration: 60, location: "Room B", description: "Desc B",
    });
    assertNotEquals("error" in createUpcomingResult, true);
    const { event: trulyUpcomingEventId } = createUpcomingResult as { event: ID };
    const uncancelUpcomingResult = await eventConcept.unCancelEvent({ organizer: organizerAlice, event: trulyUpcomingEventId });
    assertEquals("error" in uncancelUpcomingResult, true, "Should fail to un-cancel an event that is already upcoming.");
    ...
    console.log("Test: Failure - completed event.");
    const pastDateForCompletion = new Date(Date.now() - 1000 * 60 * 120); // 2 hours ago
    pastDateForCompletion.setHours(8, 0, 0, 0);
    const completedEventCreateResult = await eventConcept.createEvent({
      organizer: organizerAlice, name: "Completed Event Cannot Uncancel", date: pastDateForCompletion, duration: 60, location: "Desk", description: "Old task",
    });
    assertNotEquals("error" in completedEventCreateResult, true);
    const { event: completedEventId } = completedEventCreateResult as { event: ID };
    await eventConcept.completeEvent({ event: completedEventId }); // Mark as completed
    const completeEventCheck = await eventConcept._getEventById({ event: completedEventId });
    assertEquals(completeEventCheck.length, 1);
    assertEquals(completeEventCheck[0].status, "completed", "Setup: Event should be completed.");
    const uncancelCompletedResult = await eventConcept.unCancelEvent({ organizer: organizerAlice, event: completedEventId });
    assertEquals("error" in uncancelCompletedResult, true, "Should fail to un-cancel a completed event.");
    assertEquals((uncancelCompletedResult as { error: string }).error, "Event cannot be un-cancelled as its status is not 'cancelled'.");
    console.log("Success: Rejected un-cancellation of a completed event.");

    // --- Failure: un-cancelling an event that has already ended (cancelled in the past) ---
    console.log("Test: Failure - event has already ended (cancelled in the past).");
    const pastDate = new Date(Date.now() - 1000 * 60 * 60 * 2); // 2 hours ago
    pastDate.setHours(8, 0, 0, 0); // Normalize time
    const pastEventDuration = 30; // Event ended 1.5 hours ago
    const pastEventCreateResult = await eventConcept.createEvent({
...
Deno.test("Action: deleteEvent - success and preconditions", async () => {
  ...
  try {
    ...
    assertEquals(deletedEvent.length, 0, "Retrieving deleted event should result in an empty array.");
...
Deno.test("Action: completeEvent - success and preconditions (system action)", async () => {
  ...
  try {
    ...
    assertEquals((alreadyCompletedResult as { error: string }).error, "Event cannot be completed as its status is not 'upcoming'."); // Corrected error message
    ...
    assertEquals((completeCancelledResult as { error: string }).error, "Event cannot be completed as its status is not 'upcoming'."); // Corrected error message
...
Deno.test("Queries: _getEventById, _getEventsByOrganizer, _getEventsByStatus, _getAllEvents", async () => {
  ...
  try {
    ...
    // Setup multiple events for various queries
    ...
    const createEvent1 = await eventConcept.createEvent({ organizer: organizerAlice, name: "Alice's Upcoming Event", date: tomorrow, duration: 60, location: "Venue A", description: "Desc A" });
    const { event: aliceUpcomingEventId } = createEvent1 as { event: ID };

    const createEvent2 = await eventConcept.createEvent({ organizer: organizerBob, name: "Bob's Upcoming Event", date: tomorrow, duration: 90, location: "Venue B", description: "Desc B" });
    const { event: bobUpcomingEventId } = createEvent2 as { event: ID };

    const createEvent3 = await eventConcept.createEvent({ organizer: organizerAlice, name: "Alice's Past Event", date: yesterday, duration: 30, location: "Venue C", description: "Desc C" });
    const { event: alicePastEventId } = createEvent3 as { event: ID };
    await eventConcept.completeEvent({ event: alicePastEventId });

    const createEvent4 = await eventConcept.createEvent({ organizer: organizerBob, name: "Bob's Cancelled Event", date: tomorrow, duration: 45, location: "Venue D", description: "Desc D" });
    const { event: bobCancelledEventId } = createEvent4 as { event: ID };
    await eventConcept.cancelEvent({ organizer: organizerBob, event: bobCancelledEventId });

    console.log("Setup: Created 4 events for querying.");

    // Test _getEventById
    console.log("Query: _getEventById (existing event)");
    const fetchedAliceUpcoming = await eventConcept._getEventById({ event: aliceUpcomingEventId });
    assertEquals(fetchedAliceUpcoming.length, 1, "Should find Alice's upcoming event.");
    assertEquals(fetchedAliceUpcoming[0].name, "Alice's Upcoming Event");
    console.log("Success: Retrieved existing event by ID.");

    console.log("Query: _getEventById (non-existent event)");
    const nonExistentEventId = "event:nonexistent" as ID;
    const fetchedNonExistent = await eventConcept._getEventById({ event: nonExistentEventId });
    assertEquals(fetchedNonExistent.length, 0, "Should return an empty array for non-existent event."); // Corrected assertion
    console.log("Success: Handled non-existent event by ID (returned empty array).");

    // Test _getEventsByOrganizer
    console.log("Query: _getEventsByOrganizer (Alice)");
    const aliceEvents = await eventConcept._getEventsByOrganizer({ organizer: organizerAlice });
    assertEquals(aliceEvents.length, 2, "Alice should have 2 events (upcoming and completed).");
    assertExists(aliceEvents.find(e => e._id === aliceUpcomingEventId && e.status === "upcoming"));
    assertExists(aliceEvents.find(e => e._id === alicePastEventId && e.status === "completed"));
    console.log("Success: Retrieved events by organizer Alice.");

    console.log("Query: _getEventsByOrganizer (Bob)");
    const bobEvents = await eventConcept._getEventsByOrganizer({ organizer: organizerBob });
    assertEquals(bobEvents.length, 2, "Bob should have 2 events (upcoming and cancelled).");
    assertExists(bobEvents.find(e => e._id === bobUpcomingEventId && e.status === "upcoming"));
    assertExists(bobEvents.find(e => e._id === bobCancelledEventId && e.status === "cancelled"));
    console.log("Success: Retrieved events by organizer Bob.");

    console.log("Query: _getEventsByOrganizer (non-existent organizer)");
    const nonExistentOrganizer = "user:Charlie" as ID;
    const charlieEvents = await eventConcept._getEventsByOrganizer({ organizer: nonExistentOrganizer });
    assertEquals(charlieEvents.length, 0, "Non-existent organizer should have 0 events.");
    console.log("Success: Handled non-existent organizer.");

    // Test _getEventsByStatus
    console.log("Query: _getEventsByStatus ('upcoming')");
    const upcomingEvents = await eventConcept._getEventsByStatus({ status: "upcoming" });
    assertEquals(upcomingEvents.length, 2, "Should find 2 upcoming events.");
    assertExists(upcomingEvents.find(e => e._id === aliceUpcomingEventId));
    assertExists(upcomingEvents.find(e => e._id === bobUpcomingEventId));
    console.log("Success: Retrieved upcoming events.");
    ...
    assertEquals(cancelledEvents.length, 1, "Should find 1 cancelled event.");
    assertEquals(cancelledEvents[0]._id, bobCancelledEventId);
    console.log("Success: Retrieved cancelled events.");
    ...
    assertEquals(completedEvents.length, 1, "Should find 1 completed event.");
    assertEquals(completedEvents[0]._id, alicePastEventId);
    console.log("Success: Retrieved completed events.");
    ...
    assertExists(allEvents.find(e => e._id === aliceUpcomingEventId));
    assertExists(allEvents.find(e => e._id === bobUpcomingEventId));
    assertExists(allEvents.find(e => e._id === alicePastEventId));
    assertExists(allEvents.find(e => e._id === bobCancelledEventId));
