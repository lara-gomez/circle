...
content_id: 3845fe069f70d9b8a310ae7cb9d04548b1ecc8820ae07481baf53f1bc9b50da3
---
...
export default class EventConcept {
  ...
  async unCancelEvent({ organizer, event: eventId }: { organizer: User; event: Event }): Promise<{ event: Event } | { error: string }> {
    ...
    if (eventEndTime.getTime() < currentTime.getTime()) { // Changed from <= to < to avoid issues with exact current_time
