---
timestamp: 'Fri Nov 07 2025 13:48:37 GMT-0500 (Eastern Standard Time)'
parent: '[[../20251107_134837.5235f89e.md]]'
content_id: 2eaf870e8c4172e0998bfaa28afaa93730b28d738ccbdfbeb9dfd1e516456fd4
---

# response:

The error `Property 'user' does not exist on type '{ user: ID; } | { error: string; }'. Property 'user' does not exist on type '{ error: string; }'.deno-ts(2339)` occurs because TypeScript's static analysis isn't always able to perfectly infer the type after a runtime check like `"error" in result`.

The `_getUser` method can return either `Array<{ user: User }>` (success) or `[{ error: string }]` (failure). When you access `getUserResult[0]`, its type is a union of `{ user: User }` and `{ error: string }`.

Even after you check `"error" in getUserResult[0]`, TypeScript might not always narrow the type correctly, especially when accessing elements within an array of a union type.

To fix this, you need to explicitly tell TypeScript which type you expect after your runtime check using a type assertion `as`.
