...
content_id: 8744aa79428e4f77e9e18d26b22ab57b2069374a61937ca12082be2d408f97ae
---
...
Deno.test("Principle: User A sends request to B, B accepts, A removes B as friend", async () => {
  ...
  try {
    ...
    let aliceIncoming = await friendingConcept._getIncomingRequests({ user: userAlice });
    ...
    let bobOutgoing = await friendingConcept._getOutgoingRequests({ user: userBob });
...
Deno.test("Action: removeFriend preconditions and effects", async (t) => {
  ...
  try {
    ...
    await t.step("Fails when user tries to remove self as friend", async () => {
      // Ensure the user exists with a minimal doc for this check
      await friendingConcept.users.deleteMany({});
      await friendingConcept.ensureUserExists(userAlice);
